...
class Friend{
    ...
    public function is_already_friends($my_id, $user_id){
	 ...
	 public function is_already_blocked($my_id, $user_id){
        try{
            $sql = "SELECT * FROM `blocklist` WHERE (giver = :my_id AND taker = :frnd_id) ";
	...
	public function if_given_by_frnd($my_id, $user_id){
        try{
            $sql = "SELECT * FROM `blocklist` WHERE (giver = :frnd_id AND taker = :my_id) ";
	...
	//block frnd
	public function block_user($my_id, $user_id){
        try{
				$sql = "INSERT INTO `blocklist`(giver, taker) VALUES(?, ?)";
                $stmt = $this->db->prepare($sql);
                $stmt->execute([$my_id, $user_id]);
				$delete_friends = "DELETE FROM `friends` WHERE (user_one = :my_id AND user_two = :frnd_id) OR (user_one = :frnd_id AND user_two = :my_id)";
				$delete_stmt = $this->db->prepare($delete_friends);
	...
	//unblock req
	public function unblock_user($my_id, $user_id){
        try{
			
				$delete_friends = "DELETE FROM `blocklist` WHERE (giver = :my_id AND taker = :frnd_id) OR (giver = :frnd_id AND taker = :my_id)";
				$delete_stmt = $this->db->prepare($delete_friends);
				$delete_stmt->bindValue(':my_id',$my_id, PDO::PARAM_INT);
				$delete_stmt->bindValue(':frnd_id', $user_id, PDO::PARAM_INT);
				$delete_stmt->execute();
    ...
    public function get_all_friends($my_id, $send_data){
	   ...
	   public function get_all_blocked_friends($my_id, $send_data){
        try{
            $sql = "SELECT * FROM `blocklist` WHERE giver = :my_id ";
            $stmt = $this->db->prepare($sql);
            $stmt->bindValue(':my_id',$my_id, PDO::PARAM_INT);
            $stmt->execute();
                ...
                if($send_data){
                    ...
                    foreach($all_users as $row){
                        if($row->giver == $my_id){
                            $get_user = "SELECT id, username, user_image FROM `users` WHERE id = ?";
                            $get_user_stmt = $this->db->prepare($get_user);
                            $get_user_stmt->execute([$row->taker]);
                            array_push($return_data, $get_user_stmt->fetch(PDO::FETCH_OBJ));
                        }
                    }
