...
if(isset($_GET['action']) && isset($_GET['id'])){
    ...
    if(isset($_SESSION['user_id']) && isset($_SESSION['email'])){
        ...
        // OTHERWISE DO THIS
			...
			elseif($_GET['action'] == 'block_req'){
				$frnd_obj->block_user($my_id, $user_id);
			}
			elseif($_GET['action'] == 'unblock_req'){
				$frnd_obj->unblock_user($my_id, $user_id);
			}
			elseif($_GET['action'] == 'block_khaisi'){
				$frnd_obj->block_khaisi($my_id, $user_id);
			}
            else{
                redirect_to_profile();
            }
